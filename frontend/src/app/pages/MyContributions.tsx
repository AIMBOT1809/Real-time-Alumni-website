...
export function MyContributions() {
  ...
  if (!canContribute) return <div className="mx-auto max-w-3xl px-4 py-16 text-center"><FileText className="mx-auto h-12 w-12 text-slate-300 dark:text-slate-500" /><h1 className="mt-4 text-2xl font-bold text-slate-950 dark:text-slate-100">My Contributions</h1><p className="mt-2 text-slate-600 dark:text-slate-300">Students can browse approved posts and view their participation history. Content creation is available to alumni and faculty.</p><Link to="/dashboard" className="mt-5 inline-flex rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white">Browse posts</Link></div>;

  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:py-9">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-yellow-700 dark:text-yellow-400">Content you created</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950 dark:text-slate-100 sm:text-4xl">My Contributions</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300 sm:text-base">Track review status and manage your own posts without mixing them into participation history.</p></div><Link to="/dashboard/post" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-yellow-400 hover:text-slate-950"><Plus className="h-4 w-4" />Create Post</Link></div>

      {message && <div className="mt-5 flex items-center justify-between rounded-xl border border-slate-200 dark:border-yellow-400/20 bg-white dark:bg-slate-900/70 px-4 py-3 text-sm text-slate-700 dark:text-slate-300"><span>{message}</span><button onClick={() => setMessage('')} aria-label="Dismiss message"><X className="h-4 w-4" /></button></div>}
      ...
      {loading ? <div className="flex items-center justify-center py-20 text-slate-500 dark:text-slate-400"><Loader2 className="mr-2 h-5 w-5 animate-spin" />Loading contributions…</div> : (
        <div className="mt-5 space-y-3">
          {filtered.map((item) => {
            ...
            return <article key={item.id} className="rounded-2xl border border-slate-200 dark:border-yellow-400/20 bg-white dark:bg-slate-900/70 p-5 shadow-sm"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-slate-100 dark:bg-slate-800 px-2.5 py-1 text-xs font-bold text-slate-600 dark:text-slate-300">{typeLabel(item.type)}</span><span className={`rounded-full px-2.5 py-1 text-xs font-bold capitalize ${statusStyle[item.status]}`}>{item.status}</span></div><h2 className="mt-3 text-lg font-bold text-slate-950 dark:text-slate-100">{item.title}</h2><p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{item.content}</p><p className="mt-3 flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400"><CalendarDays className="h-3.5 w-3.5" />Created {new Date(item.createdAt).toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' })}</p></div><div className="flex shrink-0 gap-2"><button disabled={!isEditable} onClick={() => setEditing({ ...item })} title={isEditable ? 'Edit contribution' : 'Approved posts cannot be edited'} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-yellow-400/20 px-3 py-2 text-xs font-bold text-slate-700 dark:text-slate-300 transition hover:bg-slate-50 dark:hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40"><Edit3 className="h-3.5 w-3.5" />Edit</button><button disabled={!isEditable} onClick={() => deleteContribution(item)} title={isEditable ? 'Delete contribution' : 'Approved posts cannot be deleted'} className="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 dark:border-rose-400/20 px-3 py-2 text-xs font-bold text-rose-700 dark:text-rose-300 transition hover:bg-rose-50 dark:hover:bg-rose-900/20 disabled:cursor-not-allowed disabled:opacity-40"><Trash2 className="h-3.5 w-3.5" />Delete</button></div></div>{item.status === 'rejected' && item.rejectionReason && <div className="mt-4 flex items-start gap-2 rounded-xl border border-rose-200 dark:border-rose-400/20 bg-rose-50 dark:bg-rose-900/20 p-3 text-sm text-rose-800 dark:text-rose-300"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" /><div><p className="font-bold">Rejection reason</p><p className="mt-0.5">{item.rejectionReason}</p></div></div>}</article>;
          })}
        </div>
      )}

      {!loading && filtered.length === 0 && <div className="mt-5 rounded-2xl border border-dashed border-slate-300 dark:border-yellow-400/20 bg-white dark:bg-slate-900/70 py-16 text-center"><FileText className="mx-auto h-10 w-10 text-slate-300 dark:text-slate-500" /><h3 className="mt-3 font-bold text-slate-900 dark:text-slate-100">No contributions found</h3><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{search ? 'Try a broader search.' : 'Create a post to start building your contribution history.'}</p></div>}

      {editing && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/60 p-4" role="dialog" aria-modal="true" aria-labelledby="edit-contribution-title"><div className="w-full max-w-xl rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-2xl sm:p-6"><div className="flex items-center justify-between"><h2 id="edit-contribution-title" className="text-xl font-bold text-slate-950 dark:text-slate-100">Edit contribution</h2><button onClick={() => setEditing(null)} aria-label="Close edit dialog" className="rounded-lg p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"><X className="h-5 w-5" /></button></div><div className="mt-5 space-y-4"><label className="block text-sm font-bold text-slate-700 dark:text-slate-300">Title<input value={editing.title} onChange={(event) => setEditing({ ...editing, title: event.target.value })} className="mt-1.5 w-full rounded-xl border border-slate-300 dark:border-yellow-400/20 bg-white/70 dark:bg-slate-900/70 px-3 py-2.5 font-normal outline-none text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20" /></label><label className="block text-sm font-bold text-slate-700 dark:text-slate-300">Content<textarea rows={6} value={editing.content} onChange={(event) => setEditing({ ...editing, content: event.target.value })} className="mt-1.5 w-full resize-none rounded-xl border border-slate-300 dark:border-yellow-400/20 bg-white/70 dark:bg-slate-900/70 px-3 py-2.5 font-normal outline-none text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20" /></label></div><div className="mt-5 flex justify-end gap-2"><button onClick={() => setEditing(null)} className="rounded-xl px-4 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800">Cancel</button><button onClick={saveEdit} className="rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white hover:bg-yellow-400 hover:text-slate-950">Save changes</button></div></div></div>}
