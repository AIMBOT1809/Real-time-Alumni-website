...
export function AdminDashboard() {
  ...
  const adminPosts = useMemo(
    () => posts.filter((post) => post.alumniId === user?.id || post.alumniId === 'admin'),
    [posts, user?.id],
  );
...
const [highlightFiles, setHighlightFiles] = useState<File[]>([]);
  ...
  const getPostDescription = (post: any) => {
    // If there's a title, content is the full description
    if (post.title) {
      return post.content || '';
    }
    // If no title, try to get content after first line
  ...
  const handleSubmitCreatePost = async() => {
    console.log('[AdminDashboard] handleSubmitCreatePost called');
    console.log('[AdminDashboard] user:', user);
    console.log('[AdminDashboard] role:', role);
    console.log('[AdminDashboard] newPostTitle:', newPostTitle);
    console.log('[AdminDashboard] newPostDescription:', newPostDescription);

    try {
      if (!user || role !== 'admin') {
        console.warn('[AdminDashboard] Cannot create post: user or role invalid', { user: !!user, role });
        alert('You must be logged in as admin to create posts.');
        return;
      }
      ...
      if (newPostFile) {
        try {
          ...
          const filePath = `admin-posts/${fileName}`;
          ...
          if (uploadError) {
            console.error('[AdminDashboard] Image upload error:', uploadError);
            // Fallback to blob URL if upload fails
            imageUrl = URL.createObjectURL(newPostFile);
          } else {
            const { data } = supabase.storage
              .from('alumni_highlights')
              .getPublicUrl(filePath);
            imageUrl = data.publicUrl;
            console.log('[AdminDashboard] Image uploaded successfully:', imageUrl);
          }
        } catch (uploadErr) {
          console.error('[AdminDashboard] Image upload exception:', uploadErr);
          // Fallback to blob URL
          imageUrl = URL.createObjectURL(newPostFile);
        }
      }

      // Generate local ID immediately
      const postId = `admin-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      const postTimestamp = new Date().toISOString();

      // Always write to localStorage FIRST so UI updates immediately
      const newPost = {
        id: postId,
        alumniId: 'admin' as const,
        title: title,
        content: description,
        ...
        attachmentName: newPostFile?.name || undefined,
      };

      // Update the main posts localStorage (used by AuthContext and all dashboards)
      try {
        const stored = JSON.parse(localStorage.getItem('allumini_posts') || '[]');
        stored.unshift(newPost);
        localStorage.setItem('allumini_posts', JSON.stringify(stored));
        console.log('[AdminDashboard] Saved to allumini_posts, total posts:', stored.length);
      } catch (e) {
        console.error('[AdminDashboard] Failed to save to allumini_posts:', e);
      }

      // Also update the demo localStorage (used by MainDashboard)
      try {
        const demoStored = JSON.parse(localStorage.getItem('allumini_posts_demo') || '[]');
        demoStored.unshift({ ...newPost, authorName: 'Admin', authorRole: 'admin', status: 'approved' });
        localStorage.setItem('allumini_posts_demo', JSON.stringify(demoStored));
      } catch (e) {}
      ...
      // Fire-and-forget: try saving to Supabase in background (non-blocking)
      try {
        ...
        if (error) {
          console.warn('[AdminDashboard] Supabase save failed (table may not exist):', error.message);
        } else {
          console.log('[AdminDashboard] Successfully saved to Supabase admin_posts table');
        }
      } catch (err) {
        console.warn('[AdminDashboard] Supabase save exception (non-critical):', err);
      }
    } catch (error) {
      console.error('[AdminDashboard] Unexpected error in handleSubmitCreatePost:', error);
