...
export function Events() {
  const { events, getAlumniById, role, user, addEvent, isAdminId } = useAuth();
...
useEffect(() => {
  ...
  const filteredEvents = events.filter(event => {
    const isAdmin = isAdminId(event.alumniId);
    const organizerName = isAdmin ? 'Admin' : (getAlumniById(event.alumniId)?.name ?? '');
    const matchesSearch = 
          ...
          >
            ...
            <div className="p-6">
              ...
              <div className="space-y-2 text-sm text-slate-600 mb-6">
                ...
                <div className="flex items-center">
                  <Users className="h-4 w-4 text-slate-400 mr-2" />
                  <span>Organized by {isAdminId(event.alumniId) ? 'Admin' : (getAlumniById(event.alumniId)?.name ?? 'Alumni')}</span>
                </div>
              </div>
