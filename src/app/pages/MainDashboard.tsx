...
export function MainDashboard() {
  const { user, role, logout, login, posts, jobs, events, following, getAlumniById, alumni, addPost, deletePost, isAdminId } = useAuth();
  ...
  useEffect(() => {
    // Posts are already fetched by AuthContext, including admin posts
    console.log('[MainDashboard] Posts from AuthContext:', posts?.length, 'posts');
  }, [posts]);
...
*/    
 ...
 //const followedPosts = adminPosts;
  ...
  const followedPosts = [
    ...(posts || []).map((post) => ({
      ...post,
      source: post.alumniId === 'admin' ? 'admin' : 'user',
...
//};
const isAdmin = post.source === 'admin' || isAdminId(post.alumniId);
const author =
  isAdmin
    ? {
        name: 'Admin',
                        ...
                        {post.attachmentUrl && !post.image && (
                          <div className="mt-4 flex items-center gap-2 text-slate-400">
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="font-medium text-slate-300 hover:text-[#FFD700]"
                            >
                  ...
                  {(eventView === 'upcoming' ? upcomingEvents : currentEvents).map((event) => (
                    <div key={event.id} className="bg-slate-900 rounded-lg border border-slate-800 overflow-hidden hover:border-[#FFD700] transition-colors">
                      ...
                      <div className="p-6">
                        ...
                        <p className="text-slate-300 mb-4">{event.date ? new Date(event.date).toLocaleDateString() : 'TBD'} at {event.time || 'TBD'}</p>
                                ...
                                <div className="flex items-center justify-between text-sm">
                                  <div className="text-slate-400">
                                    {(() => {
                                      ...
                                      } catch { return event.date || 'Date TBD'; }
                                    })()}
                                  </div>
                                </div>
                              </div>
